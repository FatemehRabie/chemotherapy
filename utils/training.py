...
def train(algo, total_steps, num_steps, number_of_envs, seed):
    ...
    eval_callback = EvalCallback(eval_env, best_model_save_path=f'./logs_{algo}/',
                                log_path=f'./logs_{algo}/', eval_freq=128,
                                deterministic=True, render=False)
    # Initialize the logger to write both to the console and files
    new_logger = configure(folder=f"./logs_{algo}/")
    # Initialize the agent
    if algo == 'PPO':
        from stable_baselines3 import PPO
        model = PPO('MultiInputPolicy', env, n_steps=num_steps, verbose=1, gamma=1.0)
    elif algo == 'TRPO':
        from sb3_contrib import TRPO
        model = TRPO('MultiInputPolicy', env, n_steps=num_steps, verbose=1, gamma=1.0)
    elif algo == 'A2C':
        from stable_baselines3 import A2C
        model = A2C('MultiInputPolicy', env, n_steps=num_steps, verbose=1, gamma=1.0)
    else:
